...
def verificar_uno(df_uno):
    ...
    with st.expander('Errores y hallazgos:'):
        ...
        st.info('15. La columna “FECHA RETIRO” está vacía.')
        st.info('16. La columna “SISTEMA DE ENERGIA” está vacía.')
        st.info('17. La columna “CALIDAD DEL TERRENO” está vacía.')
        st.info('18. La columna “GEOLOGIA” está vacía.')
        st.info('19. La columna “TOPOGRAFIA” está vacía.')
        st.info('20. La columna “RESPONSABLE TEMATICO” está vacía.')
        st.info('21. La columna “RESPONSABLE TECNICO” está vacía.')
        st.info('22. La columna “ISOLUCION” está vacía.')
        st.info('23. La columna “ACCESO” está vacía.')
        st.info('24. El estado de la estación es "Activa", pero tiene una fecha de retiro.')
    ...
    for row_index, row in respuesta_dos.iterrows():
        ...
        if not('-' in row['CODIGO DE LOCALIZACION']):
            pass
        else:
            a = row['CODIGO DE LOCALIZACION'].split('-')
            prueba.append(a)
            ...
            if (row['ESTADO'] != 'PROXIMA A INSTALAR'):
                if not pd.notnull(row[columna]):
                    ...
                    if (numero_hallazgo == 15) and (row['ESTADO'] != 'RETIRADA'):
                        pass
                    elif (numero_hallazgo == 20) and (row['ESTADO'] != 'ACTIVA'):
                        pass
                    elif (numero_hallazgo == 21) and (row['ESTADO'] != 'ACTIVA'):
                        pass
                    else:
...
def verificar_tres(df_tres):
    ...
    with st.expander('Errores'):
        ...
        st.info('1. La estación tiene una cantidad incorrecta de "Historicos de la estación".')
        st.info('2. La estación tiene una cantidad incorrecta de "Pruebas del dataless".')
        st.info('3. La estación tiene una cantidad incorrecta de "Archivo de respuesta".')
        st.info('4. La estación no tiene ningún "Informe de calidad de la señal".')
    ...
    errores_df = pd.DataFrame(list(errores.items()), columns=['ESTACION', 'ERRORES'])
    st.dataframe(errores_df, hide_index=True, use_container_width=True)
